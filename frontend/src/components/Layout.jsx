...
const Layout = ({ onLogout, user }) => {
	...
	const fetchTasks = useCallback(async () => {
		...
		try {
			...
			const { data } = await axios.get("https://taskaro-ybh1.onrender.com/api/task/gp", {
	...
	const stats = useMemo(() => {
		const completedTasks = tasks.filter(
			(t) =>
				t.completed === true ||
				t.completed === 1 ||
				(typeof t.completed === "string" && t.comleted.toLowerCase() === "yes"),
		).length;
		...
		<div className="p-2 sm:p-3 rounded-xl bg-white shadow-sm border border-purple-100 hover:shadow-md transition-all duration-300 hover:border-purple-100 group">
			<div className="flex items-center gap-2">
				...
				<div className="min-w-0">
					<p className="text-lg sm:text-xl font-bold bg-gradient-to-br from-fuchsia-500 to-purple-600 bg-clip-text text-transparent">
						{value}
					</p>
								...
								<div className="relative pt-1">
									<div className="flex gap-1.5 items-center">
										<div className="flex-1 h-2 sm:h-3 bg-purple-100 rounded-full overflow-hidden">
											<div
												className="h-full bg-gradient-to-br from-fuchsia-500 to-purple-600 transition-all duration-500"
							...
							<div className="space-y-2 sm:space-y-3">
								{tasks.slice(0, 3).map((task) => (
									...
									>
										...
										<span
											className={`px-2 py-1 text-xs rounded-full shrink-0 ml-2 ${task.comleted ? "bg-green-100 text-green-700" : "bg-fuchsia-100 text-fuchsia-700"} `}
										>
